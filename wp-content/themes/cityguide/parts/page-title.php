...
{elseif $wp->isCategory or $wp->isArchive or $wp->isTag or $wp->isAuthor or $wp->isTax(portfolios) or $wp->isTax(items) or $wp->isTax(locations) or $wp->isTax(ait-events-pro)}
	...
	{* TITLE ********** *} {elseif $wp->isArchive}
								{if $archive->isDay}			{capture $titleName}
																	{capture $dayTitle}<span class="title-data">{$archive->rawDate}</span>{/capture}
								...
								{elseif $archive->isMonth}		{capture $titleName}
																	...
																	{capture $monthTitle}<span class="title-data">{$archive->rawDate|dateI18n: $monthFormat}</span>{/capture}
								...
								{elseif $archive->isYear}		{capture $titleName}
																	...
																	{capture $yearTitle}<span class="title-data">{$archive->rawDate|dateI18n: $yearFormat}</span>{/capture}
		...
		{/if}
			...
			<div class="entry-title {$titleClass}">
				...
				{if $wp->isSingular(event-pro)}
					{includePart portal/parts/single-event-date, meta => $post->meta('event-pro-data'), place => 'header', showYear => true, eventId => $post->id}
					{if !$options->layout->general->showBreadcrumbs}{var $headingClass = "alone"}{/if}
				{/if}
				...
				<div class="entry-title-wrap">
					...
					{/if}
					<h1 class="{if !empty($headingClass)}{$headingClass}{/if}">{!$titleName}</h1>
						...
						<div class="entry-data">
							...
							{if $dateInterval == 'yes'}
								...
								<div class="date-interval">
									...
									<time class="event-from" datetime="{$intFrom|date:'c'}">{$intFrom|dateI18n: $dateFormat}</time>
									<span class="date-sep">-</span>
									<time class="event-to" datetime="{$intTo|date:'c'}">{$intTo|dateI18n: $dateFormat}</time>
								</div>
							{/if}
			...
			{if $wp->isSingular(item) && defined('AIT_REVIEWS_ENABLED')}
