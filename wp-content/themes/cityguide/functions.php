...
add_filter('breadcrumbs_singular_taxonomy', 'setEventsProBreadcrumbsSingularTaxonomy', 10, 2);
function setEventsProBreadcrumbsSingularTaxonomy($taxonomy, $post) {
	if ($post->post_type == 'ait-event-pro') {
		return 'ait-events-pro';
	} else {
		return $taxonomy;
	}
}

function getPostPackageSlug($post_id){
	$post = get_post($post_id);

	$result = "";

	$post_package = get_post_meta($post->ID, 'post_package', true);
	if(!empty($post_package)){
		// get directly from the item meta
		$result = $post_package;
	} else {
		// retrieve it long way around
		if(!empty($post->post_author)){
			$author = new WP_User($post->post_author);
			if(isCityguideUser($author->roles)){
				$package = getCityguideUserPackage($author);
